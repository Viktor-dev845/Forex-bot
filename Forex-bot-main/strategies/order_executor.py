...
from utils.shared_types import OrderSide, OrderStatus
...
class PaperTradingExecutor(OrderExecutor):
    ...
    def __init__(self, initial_capital: float = 10000, leverage: float = 500):
        self.cash = initial_capital
        self.initial_capital = initial_capital
        self.leverage = leverage # Added leverage for realistic trading
    ...
    def _execute_order(self, order: Order, price: float) -> bool:
        ...
        # Margin model: total cost is divided by leverage
        margin_required = total_cost / self.leverage
        
        # Only check margin if we are OPENING or INCREASING a position
        # If we are reducing (closing/partial closing), no margin check needed
        is_opening = (current_qty == 0)
        is_increasing = (current_qty > 0 and order.side == OrderSide.BUY) or (current_qty < 0 and order.side == OrderSide.SELL)
        ...
        if is_opening or is_increasing:
            ...
            # We are closing or reducing. We release the proportional margin held.
            # Proportional Margin = (Qty Reduced / Total Qty) * Original Margin
            # For simplicity in this mock, we just add the margin back upon full close
            # Or proportional upon partial.
            if abs(current_qty) > 0:
                proportion = min(1.0, order.quantity / abs(current_qty))
                # Approximate original margin held for this portion
                margin_to_release = (proportion * abs(current_qty) * pos['avg_price']) / self.leverage
                self.cash += margin_to_release
            
        # Update Position logic follows...
    ...
    def get_account(self) -> dict:
        """Get account summary."""
        # In a margin account, Equity = Cash (Remaining) + Margin Held + Unrealized PnL
        # Which is equivalent to Initial Cash + Total PnL
        # For simplicity in this mock: Equity = Balance + Unrealized PnL
        
        unrealized_pnl = 0
        margin_held = 0
        for sym, pos in self.positions.items():
            current_p = self.current_prices.get(sym, pos['avg_price'])
            # PnL = (Current - Entry) * Qty
            unrealized_pnl += (current_p - pos['avg_price']) * pos['qty']
            margin_held += (abs(pos['qty']) * pos['avg_price']) / self.leverage

        # Current Balance = Initial Cash - Margin Held (simplified)
        # Actually, in our model, self.cash IS the remaining balance after margin is taken.
        total_equity = self.cash + margin_held + unrealized_pnl
