from utils.shared_types import TradeAction
...
class EnsembleStrategy(AIStrategy):
    ...
    def get_weighted_prediction(self, predictions: dict) -> float:
        """
        Calculate weighted average incorporating both:
        1. Historical Accuracy (model_weights)
        ...
        for model_name, pred in predictions.items():
            if pred is None or pred == 0.5:
                continue
            
            # 1. Base historical weight (e.g., 0.5)
            hist_weight = self.model_weights.get(model_name, 0.5)
            
            # 2. Confidence weight (0.0 to 1.0)
            # 0.51 -> 0.02 confidence | 0.99 -> 0.98 confidence
            conf_weight = abs(pred - 0.5) * 2
            
            # 3. Combined impact (Square the confidence to penalize uncertainty heavily)
            effective_weight = hist_weight * (conf_weight ** 2)
        ...
        if total_weight > 0:
            ...
            # --- Conflict Penalty ---
            # If the spread between predictions is too high, return neutral
            if len(predictions) > 1:
                max_p = max(predictions.values())
                min_p = min(predictions.values())
                spread = max_p - min_p
                
                if spread > 0.45:
                    self.logger.warning(f"CONFLICT DETECTED: Spread {spread:.2f} (Max: {max_p:.2f}, Min: {min_p:.2f}). Returning neutral 0.5.")
                    return 0.5
            
            self.logger.info(f"Confidence-weighted prediction: {final_pred:.2%} (Strength: {total_weight:.2f})")
            return final_pred
        else:
            self.logger.error("No confident model predictions available, returning neutral 0.5")
            return 0.5
