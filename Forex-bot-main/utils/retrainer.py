...
class RetrainingManager:
    ...
    def _run_retraining_script(self, symbol: str):
        ...
        try:
            ...
            if xgb_success and lstm_success:
                self.logger.info(f"[{symbol}] ✅ Full Retraining Cycle Complete (both models updated).")
                ...
                self.logger.info(f"[{symbol}] ⚠️ Partial Retraining: XGBoost updated, LSTM failed. Reloading XGBoost only.")
                self.reload_needed.add(symbol)
                # Do NOT clear history — LSTM still underperforming
            elif lstm_success:
                self.logger.info(f"[{symbol}] ⚠️ Partial Retraining: LSTM updated, XGBoost failed. Reloading LSTM only.")
                self.reload_needed.add(symbol)
            else:
